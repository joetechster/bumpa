...
// Exported so the compact inline error inside a SectionRail speaks with the
// same voice as the full-screen one - the taxonomy lives in exactly one place.
export function copyFor(error: ApiError): { title: string; detail: string } {
  switch (error.kind) {
    case 'network':
      return {
...
const styles = StyleSheet.create({
  ...
  pressed: { opacity: 0.7 },
  retryText: { ...type.body, color: colors.surface, fontWeight: '600' },
});
